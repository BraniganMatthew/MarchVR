...
// Funciton to create socket and connect to it for use in Activate funciton and runFrame with the goal of adding reconnectability
...
void ControllerDriver::RunFrame()
{
	...
	if (recv_result > 0) {
		...
		if (count <= 70) {
			temp = prev_recv;
			VRDriverLog()->Log("Prev Data!");
			count++;
		}
		else {
			temp = 0.0f;
			VRDriverLog()->Log("No Data :(");
		}
